        ...
        <section className="lg:col-span-3">
          ...
          <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1" data-testid="auto-queue-list">
            {queue.length === 0 ? (
              ...
              <div key={q.id} className="asc-card p-3 flex items-center gap-3" data-testid={`auto-queue-item-${q.id}`}>
                <StatusPill status={q.status} />
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-sm truncate">{q.topic}</div>
                  <div className="text-xs text-[var(--asc-text-muted)]">{q.kind} · {q.level || "Beginner"}{q.model_hint ? ` · ${q.model_hint}` : ""}</div>
                </div>
                {q.grades && (
                  <div className="text-[10px] text-[var(--asc-text-muted)] asc-mono" title={JSON.stringify(q.grades)}>
                    {q.grades.accuracy}/{q.grades.clarity}/{q.grades.brand_fit}/{q.grades.depth}
                  </div>
                )}
                {q.status === "needs_review" && q.draft && (
                  <button onClick={() => publishFlagged(q.id, q.topic)} className="asc-btn-secondary text-xs" data-testid={`auto-queue-publish-${q.id}`} style={{ color: "#34D399", borderColor: "rgba(52,211,153,0.5)" }}>
                    Publish anyway
                  </button>
                )}
                {q.status === "pending" && (
                  <button onClick={() => removeFromQueue(q.id)} className="text-[var(--asc-text-muted)] hover:text-[#FB7185]" data-testid={`auto-queue-remove-${q.id}`}>
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
